...
const MealKitDetail = () => {
  ...
  useEffect(() => {
    const fetchMealKit = async () => {
      const { data } = await axios(`/api/mealKits/${mealKitId}`);
      setMealKit(data);
    };
    fetchMealKit();
  }, [mealKitId]);
        ...
        <div className="w-[570px]">
          ...
          <div className="mb-8">
            ...
            <form action="" className="inline-block">
              <input defaultValue={1} max={10} min={1} type="number" className="border border-[#d3d5db] p-[6px] rounded-[4px]  w-[70px] h-[50px] leading-10 text-[18px] text-center mr-4" />
